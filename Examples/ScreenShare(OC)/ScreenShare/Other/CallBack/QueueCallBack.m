...
{
    // RLog(@"errCode:%d, cookie:%@", errCode, cookie);
...
{
    // RLog(@"queID:%d position:%d queuingTime:%d", queuingInfo.queID, queuingInfo.position, queuingInfo.queuingTime);
    dispatch_async(dispatch_get_main_queue(), ^{
...
{
    // RLog(@"queID:%d, errCode:%d, cookie:%@", queID, errCode, cookie);
    dispatch_async(dispatch_get_main_queue(), ^{
...
{
    // RLog(@"errCode:%d, usr:%@, cookie:%@", errCode, usr, cookie);
    dispatch_async(dispatch_get_main_queue(), ^{
...
{
    // RLog(@"queID:%d, usrID:%@", queID, usrID);
    dispatch_async(dispatch_get_main_queue(), ^{
